...
/**
 * Utility class for exporting any active plotting area either as Image file or hardcopy 
 * on a printer device
 */
public class PlotExportUtil {
	...
	/**
	 * Save the graph with the given filename. If the file name ends with a known extension,
	 * this is used as the file type otherwise it is the string passed in which is read from 
	 * the save as dialog form normally.
	 *
	 * @param filename the name under which the graph should be saved
	 * @param fileType type of the file
	 * @param viewerApp ViewerApp containing the current display
	 * @throws FileNotFoundException 
	 */
	public static synchronized void saveGraph(String filename, String fileType, 
											  AbstractViewerApp viewerApp) throws Exception {
		
		if (!Arrays.asList(FILE_TYPES).contains(fileType)) throw new RuntimeException("Cannot deal with file type "+fileType);
		// If they have specified the file type in the file name, use that.
		String lname = filename.toLowerCase();
		if (lname.endsWith(".png") || lname.endsWith(".jpg") || lname.endsWith(".jpeg")) fileType = FILE_TYPES[0];
		if (lname.endsWith(".ps") || lname.endsWith(".eps"))  fileType = FILE_TYPES[1];
		if (lname.endsWith(".svg"))                           fileType = FILE_TYPES[2];

		if (fileType.equals(FILE_TYPES[0])) {
			if (!lname.endsWith(".png") && !lname.endsWith(".jpg") && !lname.endsWith(".jpeg")) filename = filename+".png";
			ExportImage.exportImage(viewerApp.getCurrentViewer(), new File(filename), 1);
		
		} else if (fileType.equals(FILE_TYPES[1])) {
			if (!lname.endsWith(".ps") && !lname.endsWith(".eps")) filename = filename+".ps";
			savePostScript(new File(filename),viewerApp);
		
		} else if (fileType.equals(FILE_TYPES[2])) {
			if (!lname.endsWith(".svg")) filename = filename+".svg";
	 ...
	 * with 4 times super sampling for good quality and then sends the
	 * image to the printer, afterwards deleting the image again
	 * @param printerData SWT specific printer object
	 * @param viewerApp ViewerApp containing the current display
			...
			if (imageFile != null) {
			    ...
			    if (imageAspect > printAspect) {
					ExportImage.exportImage(viewerApp.getCurrentViewer(), imageFile, 
							area.height*printScaleFactor, area.width*printScaleFactor, oversampling);
					...
					ExportImage.exportImage(viewerApp.getCurrentViewer(), imageFile, 
							area.width, area.height, oversampling);
					ImageLoader loader = new ImageLoader();
				    imageData = loader.load(imageFile.getAbsolutePath())[0];
			    }
					...
					if (printer.startPage()) {
						...
						if (legendTable != null) {
							for (int i = 0; i < legendTable.getLegendSize(); i++)
								if (legendTable.getLegendEntry(i).isVisible()) numActiveEntries++;
						...
						if (imageAspect > printAspect) {
							...
							gc.drawImage(reloadedImage, 0, 0, imageData.width, imageData.height, -trim.x, -trim.y,
									area.width - (trim.x + trim.width), area.height - (trim.y + trim.height)-legendGap);
							if (legendTable != null) {
								int currentEntry = 0;
								for (int i = 0; i < legendTable.getLegendSize(); i++) {
									Plot1DAppearance app = legendTable.getLegendEntry(i);
